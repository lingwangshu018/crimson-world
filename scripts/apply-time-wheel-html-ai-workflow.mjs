...
source = source.replace(
  ...
  `    const item = history[index];
    ...
    const templateHtml = String(sourceModule?.template_html || item.template_html || "").trim();
    const topic = String(item.topic || "").trim();
    const extra = String(item.extra || item.run_extra || item.requirement || "").trim();
    const randomMode = !topic && !extra;

    if (!templateHtml || !/<[a-z][\\s\\S]*>/i.test(templateHtml)) {
      setStatus("该模块没有保存有效的 HTML 模板，请先进入模块编辑页填写并保存。");
      return;
    }
