...
source = source.replace(
  promptBlock,
  `      const prompt = [\n        "请读取我的绯界记录，并继续完成这一事件。",\n        "",\n        "【模块】",\n        "绯夜酒馆",\n        "",\n        "【记录编号】",\n        normalizedCode,\n        "",\n        "【记录ID】",\n        selectedRecord.id,\n        "",\n        "【读取钥匙】",\n        readKey,\n        "",\n        "【回复钥匙】",\n        noteKey,\n        "",\n        "请先调用绯界工具 crimson_read_record，使用上面的记录ID与读取钥匙精确读取这一杯酒签。",\n        "",\n        "结合当前聊天已经加载的角色卡、世界书和近期记忆，根据酒签内容继续完成随杯手记。",\n        "",\n        "【随杯手记固定格式】",\n        "───────────────────────",\n        "{地点或酒馆名} · {酒名}",\n        "随杯手记 · {副标题}",\n        "───────────────────────",\n        "",\n        "{正文}",\n        "",\n        "───────────────────────",\n        "后记",\n        "",\n        "{一至三段简短收束或余韵，不解释剧情，不概括剧情}",\n        "",\n        "· {酒馆署名} ·",\n        "───────────────────────",\n        "",\n        "【格式规则】",\n        "• 必须完整保留上述边框、标题区、正文区、后记区和末尾署名。",\n        "• 标题第一行为地点或酒馆名与酒名，第二行为‘随杯手记 · 副标题’。",\n        "• 直接进入正文，不得写‘回复如下’、‘AI’、‘以下内容’等元信息。",\n        "• 后记控制在一至三段，更像酒保留下的余韵，不得解释或总结剧情。",\n        "• 使用文学叙事与适当留白，不使用章节编号、Emoji、Markdown代码围栏或系统说明。",\n        "• 无论主题和世界观如何变化，版式不得省略、替换或改写；只填写标题、正文、后记和署名。",\n        "",\n        "完成后，请调用 crimson_write_reply，使用完全相同的记录ID与回复钥匙，将完整手记原样写回 note 字段。",\n        "",\n        "不要修改原始记录，不要创建新记录，不要写入其他记录。",\n        "只处理这一条。",\n      ].join("\\n");`,
);
